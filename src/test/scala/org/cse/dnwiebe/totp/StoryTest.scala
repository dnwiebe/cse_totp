      ...
      describe ("and given a timestamp") {
        ...
        describe ("and a client-side Totp") {
          ...
          it ("the client can be somewhat early and still match") {
            assert (clientTotp.generatePassword(identity, timestamp.minus (11, ChronoUnit.SECONDS)) ===
              serverTotp.generatePassword(identity, timestamp))
          }

          it ("the client can be somewhat late and still match") {
            assert (clientTotp.generatePassword(identity, timestamp.plus (48, ChronoUnit.SECONDS)) ===
              serverTotp.generatePassword(identity, timestamp))
          }

          it ("the client won't be authorized if it's too early") {
            assert (clientTotp.generatePassword(identity, timestamp.minus (12, ChronoUnit.SECONDS)) !==
              serverTotp.generatePassword(identity, timestamp))
          }

          it ("the client won't be authorized if it's too late") {
            assert (clientTotp.generatePassword(identity, timestamp.plus (49, ChronoUnit.SECONDS)) !==
              serverTotp.generatePassword(identity, timestamp))
          }
        }
      }
